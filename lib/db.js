...
const STANDAARD = [
  ...
  ['hero_foto', '/uploads/foto2.jpg', 'Achtergrondfoto bovenaan', 'De grote foto achter de kop. Kies een liggende foto, die staat het mooist.', 'afbeelding', 'bovenaan', 70],
  ...
  ['over_foto', '/uploads/foto13.jpg', 'Foto bij Over ons', 'Een foto van uzelf of van uw werk. Staande foto werkt hier het best.', 'afbeelding', 'over', 50],
...
//
// De negen bestaande foto's zitten in de repo onder ./uploads. Die staan niet op
// het volume, dus bij de eerste start kopieren we ze een keer over.

const BRON_UPLOADS = path.join(__dirname, '..', 'uploads');

function kopieerStartfotos() {
  if (!fs.existsSync(BRON_UPLOADS)) return;
  for (const naam of fs.readdirSync(BRON_UPLOADS)) {
    const doel = path.join(UPLOADS_DIR, naam);
    if (!fs.existsSync(doel)) {
      fs.copyFileSync(path.join(BRON_UPLOADS, naam), doel);
    }
  }
}
kopieerStartfotos();
...
const STANDAARD_PROJECTEN = [
  ['Badkamerrenovatie', '/uploads/foto1.jpg', 'Badkamerrenovatie door De Klusspecialist van Duijn in Noordwijk'],
  ['Tegelwerk', '/uploads/foto3.jpg', 'Tegelwerk in een badkamer, uitgevoerd door klusbedrijf van Duijn'],
  ['Afwerking', '/uploads/foto4.jpg', 'Strakke afwerking na een verbouwing'],
  ['Sanitair', '/uploads/foto5.jpg', 'Geplaatst sanitair in een gerenoveerde badkamer'],
  ['Renovatie', '/uploads/foto6.jpg', 'Renovatieproject in de Bollenstreek'],
  ['Interieur', '/uploads/foto7.jpg', 'Afgewerkt interieur na een klus van Piet van Duijn'],
  ['Tegelwerk', '/uploads/foto8.jpg', 'Wandtegels vakkundig gelegd'],
  ['Afwerking', '/uploads/foto9.jpg', 'Detail van de afwerking van een badkamer'],
  ['Renovatie', '/uploads/foto11.jpg', 'Complete renovatie van een woning in Noordwijk'],
];
...
module.exports = { db, q, inhoud, inhoudPerGroep, DATA_DIR, UPLOADS_DIR, DB_PATH };
